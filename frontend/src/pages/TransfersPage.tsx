import { CheckOutlined, ClockCircleOutlined, CloseOutlined, HistoryOutlined, PlusOutlined, SearchOutlined, StopOutlined } from '@ant-design/icons'
...
import { CustodyBadge, ReservationBadge } from '../components/common/CustodyBadge'
...
export function TransfersPage() {
  ...
  const create = async () => {
    ...
    setSaving(true)
    try { await transferAPI.create({ ...values, reason: values.reason || '' }); message.success('交接任务已发起，目标格位预约 30 分钟内有效'); setCreateOpen(false); createForm.resetFields(); await refresh() } finally { setSaving(false) }
  }
  const resolve = async () => {
    ...
    try {
      ...
      message.success(resolution === 'accepted' ? '样本交接已接收，预约格位转为实际占用' : resolution === 'rejected' ? '交接已拒绝，预约格位已释放' : '交接已取消，预约格位已释放')
  ...
  const specimenLocation = (specimen: Specimen) => specimen.storageContainer
    ...
    : 'intake'
  const hasReservedTarget = (row: CustodyTransfer) => Boolean(row.toContainerId && row.toPosition)
  const reservedTargetLabel = (row: CustodyTransfer) => `${row.toContainer?.code || row.toContainerId} · ${row.toPosition}`
  const columns: ColumnsType<CustodyTransfer> = [
    ...
    {
      title: '格位预约', render: (_, row) => {
        if (!hasReservedTarget(row)) return <ReservationBadge state="none" />
        return (
          <div>
            <Space size={4} wrap>
              <ReservationBadge state={row.reservationState} />
              <span>{reservedTargetLabel(row)}</span>
            </Space>
            {row.reservation && <small className="cell-subtitle">{row.reservationState === 'active' ? `有效期至 ${formatDateTime(row.reservation.expiresAt)}` : `${releaseReasonLabels[row.reservation.releaseReason || ''] || '已释放'} · ${formatDateTime(row.reservation.releasedAt || row.reservation.expiresAt)}`}</small>}
            {row.conflictReason && <small className="cell-subtitle cell-conflict">冲突：{row.conflictReason}</small>}
          </div>
        )
      },
    },
  ...
  const reservationAlert = (row: CustodyTransfer) => {
    if (!hasReservedTarget(row)) return null
    if (row.reservationState === 'active' && row.reservation) {
      return <Alert type="info" showIcon icon={<ClockCircleOutlined />} message={`已预约目标格位 ${reservedTargetLabel(row)}，有效期至 ${formatDateTime(row.reservation.expiresAt)}`} />
    }
    if (row.reservationState === 'expired') {
      return <Alert type="warning" showIcon message={`目标格位 ${reservedTargetLabel(row)} 的预约已到期释放，接收时将重新校验格位占用`} />
    }
    return <Alert type="info" showIcon message={`目标格位 ${reservedTargetLabel(row)}`} />
  }
  return (
    <div className="page-stack">
      ...
      <Modal title="发起样本交接" width={700} open={createOpen} confirmLoading={saving} onOk={() => void create()} onCancel={() => setCreateOpen(false)} okText="创建交接单" cancelText="取消">
        <Alert type="info" showIcon style={{ marginBottom: 16 }} message="发起时将校验目标容器温区与剩余容量，并预约所选格位 30 分钟；接收成功转为实际占用，拒绝、取消或到期自动释放。" />
      ...
      <Modal title={`处理交接 ${resolveTarget?.transferNo || ''}`} width={620} open={Boolean(resolveTarget)} confirmLoading={saving} onOk={() => void resolve()} onCancel={() => setResolveTarget(null)} okText="确认处理" cancelText="返回">
        <Space direction="vertical" size="large" style={{ width: '100%' }}><Segmented block value={resolution} onChange={(value) => setResolution(value as Resolution)} options={[{ value: 'accepted', label: '接收', icon: <CheckOutlined /> }, { value: 'rejected', label: '拒绝', icon: <CloseOutlined /> }, { value: 'cancelled', label: '取消', icon: <StopOutlined /> }]} />{resolution === 'accepted' && resolveTarget && reservationAlert(resolveTarget)}{resolveTarget?.conflictReason && <Alert type="error" showIcon message={`格位冲突：${resolveTarget.conflictReason}`} />}<Form form={resolveForm} layout="vertical">{resolution === 'accepted' && resolveTarget && !hasReservedTarget(resolveTarget) && <Row gutter={16}><Col span={12}><Form.Item name="toContainerId" label="目标冻存容器" rules={[{ required: true }]}><Select showSearch optionFilterProp="label" options={containers.map((item) => ({ value: item.id, label: `${item.code} · ${item.name}` }))} /></Form.Item></Col><Col span={12}><Form.Item name="toPosition" label="格位" rules={[{ required: true }]}><Input placeholder="R03-B05-C07" /></Form.Item></Col></Row>}<Form.Item name="temperatureC" label="交接复核温度 (°C)"><InputNumber min={-200} max={30} precision={1} style={{ width: '100%' }} /></Form.Item><Form.Item name="reason" label="处理说明" rules={[{ required: true, min: 3 }]}><Input.TextArea rows={3} /></Form.Item></Form></Space>
      </Modal>
