...
func SeedDemoData(ctx context.Context, db *gorm.DB) error {
	...
	return db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		acceptedBy := uint(1)
		minus80 := -78.4
		preparedOne := now.Add(-29 * time.Hour)
		reservationOneExpires := preparedOne.Add(constants.ReservationTTL)
		preparedTwo := now.Add(-5 * time.Minute)
		reservationTwoExpires := preparedTwo.Add(constants.ReservationTTL)
		transfers := []model.CustodyTransfer{
			{
				...
				ReservationStatus:    constants.ReservationConsumed,
				ReservedAt:           &preparedOne,
				ReservationExpiresAt: &reservationOneExpires,
				ReservationNote:      "接收成功，预约转为实际占用",
				State:                constants.TransferStateAccepted,
				PreparedByID:         1,
				PreparedByName:       "系统初始化",
				AcceptedByID:         &acceptedBy,
				AcceptedByName:       "系统初始化",
				PreparedAt:           preparedOne,
			...
			},
			{
				...
				ToLocation:           "样本库 B 区",
				ToContainerID:        &containers[1].ID,
				ToPosition:           "R04-BX01-C09",
				ReservationStatus:    constants.ReservationActive,
				ReservedAt:           &preparedTwo,
				ReservationExpiresAt: &reservationTwoExpires,
				State:                constants.TransferStatePrepared,
				PreparedByID:         1,
				PreparedByName:       "系统初始化",
				PreparedAt:           preparedTwo,
